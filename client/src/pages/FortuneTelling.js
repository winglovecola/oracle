...
function FuntuneTelling() {
  ...
  function question2Setup(answer) {
    if (questionNum === 1) {
      ...
      addAnswerDiv(
        <div id="answer2-div">
          <input
            id="my-question-input"
            name="my-question-input"
            type="input"
            placeholder="Input your question and hit ENTER"
            autoComplete="off"
          />
        </div>
      );
  ...
  function pickCardNum(cardnum) {
    ...
    if (cardnum === undefined || isNaN(cardnum)) return;
  ...
  function question3Setup(answer) {
    if (questionNum === 2) {
      myQuestion = answer;
      setOracleQuestion(
        'From 1 to 78, tell me the 3 numbers that first came to your mind.'
      );
  ...
  function setPlayerAnswer(elementId, questionNum, answer) {
    ...
    if (questionNum == 1) {
      if (myQuestionType === '') {
        ...
        else document.getElementById('a1-relationship').style.display = 'none';
  ...
  function meetOracle() {
    ...
    setOracleQuestion("It's destiny that we meet. What do you have in mind?");
  ...
  function audioEned() {
    if (questionNum === 1) {
      ...
      oracleListen([], {
        ...
        maxAlternatives: 1,
        continuous: true,
      }); //array from 1 to 78 as the grammar keyword
    }
  }
  ...
  function oracleListen(
    keywords = [],
    options = { interimResults: false, maxAlternatives: 1, continuous: true }
  ) {
    if (tarotCardTellingStarted) return; //cancel the voice recognition once the fortune telling started
    ...
    recognition.onresult = (event) => {
      ...
      if (questionNum === 1) {
        ...
        if (recAnswer) filterCardNum = wordsToNumbers(recAnswer);
        ...
        if (myThreeNumber.length < 3) {
          ...
          oracleListen(threeNumberSpeechGrammarArray, {
            interimResults: true,
            maxAlternatives: 1,
            continuous: true,
          });
 ...
 */
  ...
  async function tarotCardStart(pickedCardNum) {
    if (tarotCardTellingStarted) return;
  ...
  return (
    <main id="main-game-div">
      ...
      <div id="start-btn" onClick={gameStart} style={{ color: 'black' }}>
        <div className="btn-label">Explore</div>
      </div>
      <div id="user-menu">
        <div id="hpbar" className="status-bar">
          <img src="/src/img/icons/hp.png" alt="" />
        </div>
        <div id="mpbar" className="status-bar">
          <img src="/src/img/icons/mp.png" alt="" />
        </div>
      </div>
      ...
      <div id="oracle-questions">
        ...
        <div id="answer">
          {answerDiv.map((divItem, index) => (
            <div key={index}>{divItem}</div>
          ))}
        </div>
      </div>
